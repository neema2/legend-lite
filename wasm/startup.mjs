 ...
 *
 *   bazel run //wasm:startup      (on an idle machine: it times things)
 */
...
const WASM = here('./planner/classes.wasm');
const corpus = {
  model: readFileSync(here('./corpus/model.pure'), 'utf8'),
  runtime: 'trades::RT',
};
const QUERY = readFileSync(here('./corpus/queries.tsv'), 'utf8').split('\n')
  .find((l) => l.startsWith('pipeline\t')).slice('pipeline\t'.length);
