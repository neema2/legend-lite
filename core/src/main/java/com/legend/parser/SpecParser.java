 ...
 *
 * <h2>Phase status &mdash; C.1 (leaf expressions)</h2>
 * The grammar currently recognises:
 * <ul>
 *   <li>Numeric literals &mdash; integer ({@link CInteger}, narrowing to
 *       {@link Long} when the value fits else {@link BigInteger}), float
 *       ({@link CFloat}), decimal ({@link CDecimal}, source suffix
 *       {@code d}/{@code D}).</li>
 *   <li>String literals ({@link CString}) &mdash; quotes stripped,
 *       backslash escapes resolved.</li>
 *   <li>Boolean literals ({@link CBoolean}) &mdash; {@code true} /
 *       {@code false}.</li>
 *   <li>Temporal literals &mdash; {@link CDateTime}, {@link CStrictDate},
 *       {@link CStrictTime}, {@link CLatestDate} (the {@code %latest}
 *       sentinel).</li>
 *   <li>Variable references ({@link Variable}) &mdash; {@code $name}.</li>
 *   <li>Collection literals ({@link PureCollection}) &mdash;
 *       {@code [v1, v2, ...]}, including the empty form {@code []}.</li>
 * </ul>
 *
 * <p>Anything else &mdash; property access, function application,
 * operators, lambdas, {@code let}, code blocks, milestoning &mdash;
 * raises a {@link ParseException} that names the unsupported construct
 * by token type. Per the AGENTS.md no-fallbacks rule, the parser never
 ...
 * directly. Subsequent phases (C.2 &ndash; C.5) progressively extend
...
public final class SpecParser {
    ...
    /**
     * Parse a single expression starting at the current cursor. Dispatches
     * on the leading token type; later phases will widen this to a
     * Pratt-style precedence parser when operators land.
     */
        ...
        return switch (t) {
            ...
            default -> {
                ElementParser.throwAt(tokens, pos,
                        "unsupported expression token in C.1 grammar: " + t
                        + " ('" + safeText(tokens, pos) + "')");
