...
/**
 * A planner that rewrites the snapped table into the SQL legend-lite
 * produced for the live source.
 *
 * While snapped, the rows live in a local table rather than behind the
 * mapping, but the QUERY is identical -- same grouping, same
 * aggregates, same ordering. Rather than plan twice, the live plan is
 * produced once and its base table swapped. That keeps one planner and
 * makes snapped and live provably the same query modulo the source.
 */
export class SnapRewritingPlanner implements Planner {
  readonly #inner: Planner;
  readonly #liveTable: string;
  readonly #resolveSnapTable: () => string | null;

  constructor(
    inner: Planner,
    liveTable: string,
    resolveSnapTable: () => string | null,
  ) {
    this.#inner = inner;
    this.#liveTable = liveTable;
    this.#resolveSnapTable = resolveSnapTable;
  }

  async plan(pureGrammar: string, snapshot: CubeSnapshot): Promise<string> {
    const sql = await this.#inner.plan(pureGrammar, snapshot);
    const snapTable = this.#resolveSnapTable();
    if (!snapTable) return sql;
    // Replace the base table only where it appears as a FROM/JOIN
    // target, never inside a column name or a string literal.
    const pattern = new RegExp(
      `(\\b(?:FROM|JOIN)\\s+)${escapeRegExp(this.#liveTable)}\\b`,
      'gi',
    );
    return sql.replace(pattern, `$1"${snapTable}"`);
  }
}
