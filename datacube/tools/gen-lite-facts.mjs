...
// build failure (the //datacube:update_generated diff test) rather than
// a wrong number in a grid.
//
...
//
// A BUILD ACTION (datacube/BUILD.bazel, lite_facts): Bazel hands it
// core's Java sources and where to write, and the diff test compares
// its output with the committed file.
//
//   bazel run //datacube:update_generated    # rewrite the committed copy
//
// Usage: node tools/gen-lite-facts.mjs <core-java-root> <out-file>
//   <core-java-root> is the directory holding com/legend/.

import { readFileSync, writeFileSync } from 'node:fs';
import { join, resolve } from 'node:path';

if (process.argv.length !== 4) {
  console.error('usage: gen-lite-facts.mjs <core-java-root> <out-file>');
  process.exit(2);
}
const CORE = resolve(process.argv[2], 'com/legend');
...
function source(rel, fact) {
  try {
    ...
    throw new Error(`cannot read ${rel} — ${fact} comes from there. `
      + `Was it given core's Java source root?`);
  }
}
...
console.log(`lite facts: separator ${JSON.stringify(separator)}, `
