...
Every performance number in `model/` was taken against
...
Mirrors `model/cachehost.py` exactly (10M rows x 6 columns,
...
model from `model/cellcost.py` holds in WASM as well, at
...
`model/README.md` ("start simple") is safe precisely because
