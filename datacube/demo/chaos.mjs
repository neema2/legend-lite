...
try {
  ...
  console.error(`legend-lite is not answering on ${ENGINE} — start it with`
    + ` \`bazel run //core:server\`. (${e.message})`);
  process.exit(1);
}
 ...
 */
// A harness race is expected under abuse and not a finding in itself --
// but a swallowed error is also how a stale selector hides, so it is
// SAID, one line each, where a failing check can be read against it.
const tolerant = async (fn) => {
  try {
    await fn();
  } catch (e) {
    // Playwright's call log says WHY an action timed out (covered,
    // hidden, detached); the last such line is the one worth reading.
    const lines = String(e?.message ?? e).split('\n');
    const why = lines.filter((l) => /intercepts|not visible|not stable|detached|not enabled/.test(l)).pop();
    console.log(`  (tolerated: ${lines[0]}${why ? ` — ${why.trim()}` : ''})`);
  }
};

// Abuse opens WINDOWS -- a double-click on a cell drills through, into
// a floating window over the grid -- and a person closes one before
// reaching past it. Without this every later right-click lands on the
// window instead (Playwright: `.dc-drill ... intercepts pointer
// events`), and a stale harness reads as a dead context menu.
const closeWindow = async () => {
  const close = page.locator('.dc-app-overlay:not([hidden]) .dc-overlay-close');
...
async function storm(actions, rounds, label, { onGrid = true } = {}) {
  const used = new Map();
  for (let round = 0; round < rounds; round++) {
    // A storm on the GRID closes whatever window the last burst opened
    // (a drill-through, a dialog) -- otherwise one early double-click
    // leaves the rest of the storm bouncing off that window, and the
    // grid is never abused at all. The editor storm works INSIDE its
    // window, so it keeps it.
    if (onGrid) await closeWindow();
...
if (editorOpened) {
  ...
  await storm(EDITOR_ACTIONS, Math.ceil(ROUNDS / 2), 'fired', { onGrid: false });
  await tolerant(async () => {
