 ...
 * ArrowArray, duckdb_data_chunk_to_arrow) into the heap, one bulk copy per
 * buffer, as a {@link Columnar}: what the API's value rules, DuckDB's nested
 * text and the Arrow framer read. Layouts, as DuckDB's conversion writes them (measured,
...
final class ColumnData {
    ...
    static Columnar copy(MemorySegment array, TypeTree tree) {
        ...
        return switch (tree.type) {
            case DuckType.ListOf l -> tree.id == Duck.ARRAY
                    ? new Columnar(tree.type, n, nulls, validity, EMPTY, null, new byte[0], kids, false)
                    : new Columnar(tree.type, n, nulls, validity, EMPTY, ints(bufs.get(ADDRESS, 8), n + 1), new byte[0], kids, false);
            case DuckType.MapOf m -> new Columnar(tree.type, n, nulls, validity, EMPTY, ints(bufs.get(ADDRESS, 8), n + 1), new byte[0], kids, false);
            case DuckType.StructOf s -> new Columnar(tree.type, n, nulls, validity, EMPTY, null, new byte[0], kids, false);
            case DuckType.Scalar s -> scalar(tree, s.base(), n, nulls, validity, bufs);
        };
    }

    private static Columnar scalar(TypeTree tree, String base, int n, int nulls, byte @Nullable [] validity,
            MemorySegment bufs) {
        return switch (base) {
            case "VARCHAR", "JSON", "UUID", "BLOB" -> {
                int[] offs = ints(bufs.get(ADDRESS, 8), n + 1);
                yield new Columnar(tree.type, n, nulls, validity, EMPTY, offs, bytes(bufs.get(ADDRESS, 16), offs[n]), List.of(), false);
            }
            case "BOOLEAN" -> new Columnar(tree.type, n, nulls, validity, le(bytes(bufs.get(ADDRESS, 8), (n + 7) / 8)), null,
                    new byte[0], List.of(), false);
            default -> new Columnar(tree.type, n, nulls, validity, le(bytes(bufs.get(ADDRESS, 8), (long) n * Columnar.width(base))),
                    null, new byte[0], List.of(), false);
        };
    }

    private static Columnar enumColumn(TypeTree tree, int n, int nulls, byte @Nullable [] validity, MemorySegment bufs,
            MemorySegment dict) {
        ...
        return new Columnar(tree.type, n, nulls, validity, EMPTY, offs, text.toByteArray(), List.of(), false, index);
    }
