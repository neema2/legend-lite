 ...
 * quoting rules. Used for a nested column's cells and for INTERVAL.
 *
 * <p>A few C calls per value: only nested columns and intervals pay them.
 */
final class DuckValues {
    ...
    static String text(Duck d, ColumnData c, int row) {
        try (Arena a = Arena.ofConfined()) {
            MemorySegment v = value(d, c, row, a);
            try {
    ...
    /** An INTERVAL's text, as DuckDB spells it ("1 day 02:00:00"). */
    static String interval(Duck d, int months, int days, long micros) {
        try (Arena a = Arena.ofConfined()) {
            MemorySegment in = a.allocate(Duck.INTERVAL);
            in.set(JAVA_INT, 0, months);
            in.set(JAVA_INT, 4, days);
            in.set(JAVA_LONG, 8, micros);
            MemorySegment v = (MemorySegment) d.createInterval.invokeExact(in);
            try {
                return d.owned((MemorySegment) d.getVarchar.invokeExact(v));
            } finally {
                destroy(d, v, a);
            }
        } catch (Throwable t) {
    ...
    private static MemorySegment value(Duck d, ColumnData c, int row, Arena a) throws Throwable {
        ...
        return switch (t.type) {
            case DuckType.ListOf l -> {
                ...
                ColumnData items = c.children.get(0);
                MemorySegment vs = a.allocate(ADDRESS, Math.max(1, to - from));
                for (int i = from; i < to; i++) vs.setAtIndex(ADDRESS, i - from, value(d, items, i, a));
                MemorySegment out = size > 0
                        ? (MemorySegment) d.createArray.invokeExact(items.tree.logical, vs, (long) (to - from))
                        : (MemorySegment) d.createList.invokeExact(items.tree.logical, vs, (long) (to - from));
            ...
            case DuckType.StructOf s -> {
                ...
                for (int i = 0; i < n; i++) vs.setAtIndex(ADDRESS, i, value(d, c.children.get(i), row, a));
            ...
            case DuckType.MapOf m -> {
                ...
                for (int i = from; i < to; i++) {
                    ks.setAtIndex(ADDRESS, i - from, value(d, c.children.get(0), i, a));
                    vs.setAtIndex(ADDRESS, i - from, value(d, c.children.get(1), i, a));
                }
            ...
            case DuckType.Scalar s -> scalar(d, c, row, s.base(), a);
        };
    }

    private static MemorySegment scalar(Duck d, ColumnData c, int row, String base, Arena a) throws Throwable {
        ...
        return switch (base) {
            ...
            case "DECIMAL" -> {
                ...
                dec.set(JAVA_BYTE, 0, (byte) c.tree.precision);
                dec.set(JAVA_BYTE, 1, (byte) c.tree.scale);
            ...
            case "ENUM" -> (MemorySegment) d.createEnum.invokeExact(c.tree.logical,
