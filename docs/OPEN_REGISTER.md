...
| F18 | UNTANGLE (UPSTREAM_BOUNDARY_PROGRAM.md §3 D, rewritten 2026-09-24): steps 0–2 landed (guardrail pins, catalog diff 0 divergent / 191 missing, declaration + implementation tables total over 3,157 rows, 0 dangling/0 conflicts). OPEN: step 3 shadow diff at the two decision points (`FunctionCompiler.functionsAt`, lowering's pick) incl. the `indexOf` channel-B regression cause; step 4 switch-and-delete (PCT rule, `isPlatformOwnedFunction`, `CORE_FUNCTION_PACKAGES`, bare-name `nativeKeysAt`, `CoreFn.of` fallback, `KNOWN_ABSENT`) — first switch is the CHECKPOINT; 2b stdlib-resource decision by measurement at step 4; step 5 identity pins → 0, `native-membership.tsv`/`native-claims.tsv` retire; the 89 `CoreFn.OWNS` entries reviewed (derived mechanically); #23 SQL sharing is on the critical path once Body inlining widens | L |
