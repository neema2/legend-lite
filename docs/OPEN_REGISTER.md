...
| P6 | **LEG V — the vocabulary/structure separation. RECOMMENDED 2026-09-16 (user-directed), NOT RATIFIED.** Charter: [H2_PARITY_CENSUS_2026_09_16.md](H2_PARITY_CENSUS_2026_09_16.md) §5.14; already a deferred slice of [CARRIER_REDESIGN.md](CARRIER_REDESIGN.md) §5, now with MEASURED scope. `AnsiSqlRenderer` conflates two jobs — traversal (rightly shared) and vocabulary (never shareable) — and the second is the root cause of **~35 silent emissions** (§2.5): DuckDB function names rendered into H2 SQL, dying at execution with `Function "MAP_KEYS" not found` instead of raising a loud `DialectCapability`. **SQLite is the second victim** (`Compiler.java:730-733` hands it `Spellings.DUCKDB`; `SQLiteIntegrationTest` is in gate 1), so this is a platform defect with two witnesses, not an H2 inconvenience. Root line: `Spellings.h2()` starts from `build()`. Work: V1 empty H2 map (closes 24, fold into phase B) · V2 relocate 8 DuckDB-spelled base arms to `DuckDb` · V3 hooks for `OrderedListAgg`/Map cast/Array cast (`T[]` is a SYNTAX ERROR on H2 — it spells `INTEGER ARRAY`) · V4 purge 8 vocabulary literals from the base (incl. `rowid`) · V5 re-point 6 `EngineStyleH2.super.call()` fall-throughs · V6 rename to `SqlRenderBase` (the name invites the bug). Acceptance: base-literal scan + **DuckDB lane byte-unchanged at 2477/1247** (the oracle that makes this LOW risk, not high) + golden byte-parity. **Buys ZERO passing tests** — it buys trustworthy measurement and a cheap next backend (`DUCKDB_FUNCTION_COVERAGE` prices six planned backends at ~1,770 render arms, all inheriting this defect). Sequence after Z/A/C, before any new backend. Explicitly NOT "real ANSI" (no engine executes it, so conformance is asserted by nothing) and NOT one-renderer-per-dialect (~7,000 duplicated traversal lines; F10 slice 1 `079ebfda` already collapsed exactly that) | L |
| P5 | **H2 parity (END_TO_END_PLAN Phase 7) — CENSUSED 2026-09-16, [H2_PARITY_CENSUS_2026_09_16.md](H2_PARITY_CENSUS_2026_09_16.md).** Gap measured at HEAD: PCT 246 of 1249, corpus 332 of 2613, sets nest exactly (0 the other way); the lane is DIVERGING (H2 +3 since 2026-08-01, DuckDB +~300). Eight sequenced steps in census §5. **Two are S and independent of the rest:** (a) the `sessionSetup()` UDF seam on `H2`/`H2Modern` — a LIVE PRODUCTION DEFECT, `H2.splitPartCall` emits `legend_h2_extension_split_part` which only the test harness ever registers, so it cannot resolve on a connection the product opens; (b) widen gate 7 from Relation-only to all five PCT suites — 219 of the 246 gap rows are in NO gate today. Then Tier 0 spellings (~31 rows: H2 has `BITAND`/`LSHIFT` and every collection-reduction aggregate natively) → Tier 1 scalar UDFs (~104, nine already written in `H2ExtensionFunctions` at test scope) → codec fixes (~64, no SQL) → Tier 2 array UDFs (~151, `Integer[]` in/out proven) → the carrier flip + ordinal-join explode + lambda lowering (~109). **Blocking correction:** `CarrierStrategies.Caps.H2.nativeLists=false` is wrong — H2 2.4.240 has a full native ARRAY type (jagged nesting and `CARDINALITY` both correct); `BACKEND_PORTABILITY` §3's contrary evidence is Postgres's, and its "H2 has none" for correlated explosion is false (cardinality-bounded `SYSTEM_RANGE` ordinal join, empty collections preserved — probed) | L |
