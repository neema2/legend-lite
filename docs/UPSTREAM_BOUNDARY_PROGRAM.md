...
| 4 | **Switch and delete, one consumer at a time — in the order the census dictates.** (4a) the PICK first: the inliner and the lowering consult `ImplementationTable.of(id)` before inlining a body — a bodied declaration with an Intrinsic row lowers by its rule, never by its body (this is what made the `max[1..*]` admission regress); `isPlatformImplementedDerived` becomes a registration. (4b) the resolver's universe gains every platform function FQN, so natives qualify through `CORE_IMPORTS` like user functions — BARE-NAME → 0; the catalog's bare index and the `CORE_FUNCTION_PACKAGES` courtesy die. (4c) the overload set from `DeclarationTable.at` — the PCT-twin rule and `isPlatformOwnedFunction` die; the 29 TABLE-MORE FQNs become visible and the missing overloads are registered by id (same rule, more ids). (4d) `CoreFn.of`'s name-tail fallback and `Scalars.KNOWN_ABSENT`. Each switch deletes the proxy behind it in the same commit, with the shadow re-run before and after. The missing 191 overloads enter the declaration table as upstream's bodies (2b: whole stdlib resource vs library-only is decided HERE, by measurement). **CHECKPOINT: the first switch lands green with no new special case, or the program stops and the design is wrong** | corpus/PCT/census counts (expect PCT UP: suppressed bodies run); guardrail pins DOWN | |
