...
| **LEG V — vocabulary/structure separation** (the ANSI base purification) | **RECOMMENDED 2026-09-16 (user-directed), NOT RATIFIED** — charter [H2_PARITY_CENSUS_2026_09_16.md](H2_PARITY_CENSUS_2026_09_16.md) §5.14, register P6; already a deferred slice of [CARRIER_REDESIGN.md](CARRIER_REDESIGN.md) §5, now MEASURED | `AnsiSqlRenderer` conflates traversal (rightly shared) with vocabulary (never shareable). The second causes **~35 silent emissions** — DuckDB names rendered into H2 SQL, dying as `Function "X" not found` instead of a loud `DialectCapability`. **SQLite is the second victim and sits in gate 1.** Root line: `Spellings.h2()` starts from `build()`. The base is only 1,285 lines and its vocabulary contamination is 8 enumerable literals (incl. `rowid`, the DuckDB-ism §5 already named). Acceptance = base-literal scan + DuckDB lane byte-unchanged (2477/1247, the oracle that makes this LOW risk) + golden byte-parity. Buys ZERO passing tests; buys trustworthy measurement and a cheap 7th backend. NOT "real ANSI" (nothing executes it) and NOT one-per-dialect (F10 slice 1 already collapsed that) |
