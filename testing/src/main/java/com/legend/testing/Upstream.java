 ...
 * {@code upstream = True}); Maven forwards the root pom's {@code legend.engine.root}
 * / {@code legend.pure.root} properties to every test JVM. Each test used to repeat
    ...
    private static Path root(String property) {
        ...
        if (value == null || value.isEmpty()) {
            throw new IllegalStateException("-D" + property + " is not set — run the test"
                    + " through Bazel (a target with upstream = True) or Maven, which set it");
        }
