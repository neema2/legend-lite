 ...
 *
 * <p>asOfJoin is a temporal/inequality join: the match condition ({@code $l.time > $r.time})
 ...
 *   <li>Match + key: 4-param {@code asOfJoin(left, right, matchCondition, keyCondition)}</li>
 *   <li>Match operators: {@code >}, {@code >=}, {@code <}, {@code <=}</li>
 *   <li>Prefix variants: with/without column deduplication</li>
 *   <li>Chained operations: asOfJoin→filter, asOfJoin→select, asOfJoin→extend</li>
 *   <li>Edge cases: empty tables, no match, single row, closest match, NULL verification</li>
 * </ul>
 */
public class AsOfJoinCheckerTest extends AbstractDatabaseTest {
    ...
    class MatchWithKey {
        ...
        void testAsOfJoinWithKeyConditionTimestamps() throws SQLException {
            ...
            assertEquals(3, result.rowCount());
            var byTrade = collectResults(result, "trade_id", "price");
            // trade_id=1 AAPL 10:30 → closest AAPL quote before 10:30 = A(10:00) → price=180
            assertEquals(180L, ((Number) byTrade.get(1)).longValue(), "AAPL trade matched AAPL quote");
            // trade_id=2 MSFT 10:30 → closest MSFT quote before 10:30 = B(10:00) → price=350
            assertEquals(350L, ((Number) byTrade.get(2)).longValue(), "MSFT trade matched MSFT quote");
            // trade_id=3 AAPL 11:30 → closest AAPL quote before 11:30 = C(11:00) → price=182
    ...
    class PrefixDeduplication {
        ...
        void testAsOfJoinOverlappingWithPrefix() throws SQLException {
            ...
            var row0 = result.rows().get(0);
            var row1 = result.rows().get(1);
            // Find which row has left ts=10 vs ts=20
            for (var row : result.rows()) {
                long ts = ((Number) row.get(leftTsIdx)).longValue();
                if (ts == 10) assertEquals(50L, ((Number) row.get(rightValIdx)).longValue());
                if (ts == 20) assertEquals(150L, ((Number) row.get(rightValIdx)).longValue());
            }
        }
        ...
        void testAsOfJoinPartialOverlapPrefix() throws SQLException {
            ...
            for (var row : result.rows()) {
                ...
                if (ts == 10) assertEquals(90L, ((Number) row.get(scoreIdx)).longValue(), "ts=10 → score=90");
                if (ts == 20) assertEquals(85L, ((Number) row.get(scoreIdx)).longValue(), "ts=20 → score=85");
            }
        }
        ...
        void testAsOfJoinMatchKeyWithPrefix() throws SQLException {
            ...
            for (var row : result.rows()) {
                ...
                if ("AAPL".equals(sym)) assertEquals(150L, ((Number) row.get(priceIdx)).longValue());
                if ("GOOG".equals(sym)) assertEquals(2810L, ((Number) row.get(priceIdx)).longValue());
            }
        }
    }
