 ...
 * resolved metadata (including resolved call targets on
 * {@code TypedUserCall.callee} and {@code TypedGetAll.mappingFn}) directly on
 * each variant. There is no AST sidecar, no identity-keyed map; downstream
 * passes (MappingResolver, PlanGenerator) traverse the HIR via structural
 * pattern matching.
 *
